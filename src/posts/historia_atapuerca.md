---
layout: post.njk
title: "Atapuerca y la evolución humana"
excerpt: "Ficha de contexto sobre los yacimientos de la Sierra de Atapuerca y lo que se ha encontrado en ellos. Pendiente de redactar."
modulo: "Historia"
hero: "/images/atapuerca2-800.jpg"
alt: "Una excavación arqueológica al aire libre protegida por una gran marquesina metálica: los cortes del terreno dejan a la vista los estratos superpuestos y las cuadrículas de excavación, con el valle boscoso y las sierras al fondo bajo luz de atardecer"
templateEngineOverride: njk,md
permalink: /historia/historia_atapuerca/
---

*Fichas relacionadas de la agenda:*
{% wiki "2026-08-19-atapuerca", "Día 1 — Yacimientos de Atapuerca" %}
{% wiki "historia_megalitismo", "Los dólmenes de Burgos — 5.000 años" %}

> ⚠️ **Ficha pendiente de redactar y revisar.** Es un esqueleto: las cifras, dataciones y nombres de especies están **sin verificar** y hay que contrastarlos con las fuentes antes de darlos por buenos.

## Qué contar aquí

- **Qué es la Sierra de Atapuerca** — el conjunto de yacimientos y por qué es excepcional (secuencia continuada, estado de conservación).
- **La Trinchera del Ferrocarril** — Gran Dolina, Galería y Sima del Elefante: qué se ve en la visita guiada y qué se ha excavado en cada una.
- **Las especies** — *Homo antecessor*, *Homo heidelbergensis*, y qué aporta la **Sima de los Huesos**. *Dataciones por verificar.*
- **Patrimonio de la Humanidad** — declaración UNESCO (año *por verificar*) y qué implicó.
- **Dónde están los fósiles** — relación entre yacimiento (Atapuerca), CAREX y el MEH de Burgos.

## Fuentes por revisar

- Fundación Atapuerca: <https://www.atapuerca.org/>
- Museo de la Evolución Humana: <https://www.museoevolucionhumana.com/>
- Yacimientos de Atapuerca (Wikipedia): <https://es.wikipedia.org/wiki/Yacimientos_de_Atapuerca>
- *Homo antecessor* (Wikipedia): <https://es.wikipedia.org/wiki/Homo_antecessor>
- Sima de los Huesos (Wikipedia): <https://es.wikipedia.org/wiki/Sima_de_los_Huesos>

## Tareas pendientes

- Redactar el texto y **verificar cada dato** antes de publicarlo.
